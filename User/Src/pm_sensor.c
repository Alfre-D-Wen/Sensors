...
void PM_Read_Data(void) //读取微粒测量结果
{
    ...
    HAL_UART_Transmit(&SENSOR_ADDRESS, PM_READ_DATA_COMMAND, 4, 1000);
    HAL_UART_Receive_DMA(&SENSOR_ADDRESS, PM_RX_DATA_BUFFER, 8);
    PM_2_5_DATA = pm_25_bit1 * 256 + pm_25_bit0;
}

void PM_Stop_Transmit_Auto(void) //传感器停止自动发送
{
    HAL_UART_Transmit(&SENSOR_ADDRESS, PM_STOP_TRANS_AUTO_COMMAND, 4, 1000);
    HAL_UART_Receive_DMA(&SENSOR_ADDRESS, PM_RX_RESPOND_BUFFER, 2);
}

void PM_Start_Transmit_Auto(void) //传感器开启自动发送
{
    HAL_UART_Transmit(&SENSOR_ADDRESS, PM_START_TRANS_AUTO_COMMAND, 4, 1000);
    HAL_UART_Receive_DMA(&SENSOR_ADDRESS, PM_RX_RESPOND_BUFFER, 2);
}
